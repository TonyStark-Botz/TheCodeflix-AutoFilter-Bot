import sys, pytz, glob, importlib, os, logging, asyncio
...
async def auto_restart():
    """Function to restart the bot every 24 hours."""
    while True:
...
async def cache_cleanup():
    """Periodically cleans up in-memory caches to prevent memory leaks."""
    MAX_SIZE = 500
    while True:
        await asyncio.sleep(30 * 60)  # Run every 30 minutes
        try:
            # Clean temp class caches
            for attr in ['GETALL', 'SHORT', 'SETTINGS', 'IMDB_CAP', 'VERIFY', 'KEYWORD', 'SEND_ALL_TEMP', 'DEL_MSG', 'MELCOW']:
                cache = getattr(temp, attr, {})
                if isinstance(cache, dict) and len(cache) > MAX_SIZE:
                    keys_to_remove = list(cache.keys())[:len(cache) - MAX_SIZE]
                    for k in keys_to_remove:
                        cache.pop(k, None)
            logging.info("Cache cleanup done successfully.")
